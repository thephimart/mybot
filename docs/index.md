# mybot Documentation

Welcome to mybot documentation.

## Getting Started

- [Installation](installation.md) - Install mybot
- [Quick Start](quickstart.md) - Setup and first message
- [CLI Commands](cli.md) - All available commands

## Configuration

- [Configuration Reference](configuration.md) - Full config.json schema
- [Providers](providers.md) - LLM providers (OpenAI, Claude, Ollama, etc.)
- [Channels](channels.md) - Telegram and Email setup
- [Agent Settings](agent.md) - Model, temperature, memory

## Customization

- [Workspace Files](workspace.md) - AGENTS.md, SOUL.md, USER.md, TOOLS.md
- [Skills](skills.md) - Extending agent capabilities
- [Tools](tools.md) - Tool configuration

## Services

- [Cron](cron.md) - Scheduled tasks
- [Heartbeat](heartbeat.md) - Periodic agent wake-up
- [Gateway Restart](restart.md) - Agent-requested restarts via systemd

## Reference

- [LLM Parameters](parameters.md) - LiteLLM parameters
- [Security](security.md) - Security best practices

---

For project overview, see [README.md](../README.md)
